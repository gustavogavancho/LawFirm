...
public class UpdateCaseCommandHandler : IRequestHandler<UpdateCaseCommand>
{
    ...
    public async Task Handle(UpdateCaseCommand request, CancellationToken cancellationToken)
    {
        ...
        var existingCase = await _caseRepository.GetByIdAsync(request.Id, false, x => x.Clients, x => x.CounterParts, x => x.Events, x=> x.Charges, x=> x.Notifications, x => x.Statuses, x => x.Notes, x => x.ConsultingFee);
